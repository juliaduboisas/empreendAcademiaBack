...
def runAIPipeline():
    ...
    #
    #     pred_proba = model.predict_proba(X_test)[:, 1]  # Probability of Class 1
    #
    #     # 1. Prepare the results DataFrame
    #     X_test_results = X_test.copy().reset_index(drop=True)
    #     y_test_results = y_test.copy().reset_index(drop=True)
    #
    #     X_test_results["True_Status"] = y_test_results
    #     X_test_results[f"{name}_Prediction"] = pred
    #     X_test_results[f"{name}_Prob_1"] = pred_proba
    #
    #     # 2. Define the output filename based on the model name
    #     output_filename = f"{name.replace(' ', '_')}_predictions.csv"
    #
    #     # 3. Save the DataFrame to CSV
    #     # index=False prevents Pandas from writing the DataFrame's internal row number
    #     # as the first column in the CSV file.
    #     X_test_results.to_csv(output_filename, index=False)
    #
    #     print("\n====================")
    #     print(f"MODELO: {name}")
    #     print("====================")
